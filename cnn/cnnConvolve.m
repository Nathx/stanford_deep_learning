function convolvedFeatures = cnnConvolve(filterDim, numFilters, convImages, W, b)
...
numImages = size(convImages, 3);
imageDim = size(convImages, 1);
...
for imageNum = 1:numImages
  for filterNum = 1:numFilters
    ...
    % Obtain the image
    im = squeeze(convImages(:, :, imageNum));

    % Convolve "filter" with "im", adding the result to convolvedImage
    % be sure to do a 'valid' convolution

    convolvedImage = conv2(convImages(:,:,imageNum), filter, shape = "valid")
